...
export function ServicesSection() {
  ...
  return (
    <section id="services" ref={containerRef} className="py-28 lg:py-36 px-8 lg:px-12 xl:px-16 bg-[#0A0A0A] text-brand-light relative overflow-hidden">
      <div className="max-w-[1600px] mx-auto w-full flex flex-col lg:flex-row gap-16 lg:gap-20">
        ...
        <div className="lg:w-[38%] flex flex-col justify-between self-stretch">
          <div>
            {/* Step & Category Tag (Top aligned with right cards) */}
            <div className="flex items-center gap-3 mb-10 lg:mb-14">
              <span className="text-brand-yellow text-xs font-semibold tracking-widest flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-brand-yellow"></span>
                {t.services.step}
              </span>
              <span className="text-xs font-semibold tracking-widest uppercase text-brand-gray/80">
                {t.services.badge}
              </span>
            </div>
            
            {/* Main Section Title */}
            <h2 className="text-4xl sm:text-5xl lg:text-[56px] font-bold leading-[1.1] mb-6 tracking-tight text-white">
            ...
            <div>
              <a
                href="#services"
                className="group inline-flex items-center gap-2.5 text-xs font-bold uppercase tracking-widest text-brand-yellow hover:text-white transition-colors pb-1 border-b-2 border-brand-yellow hover:border-white"
              >
          ...
          {/* Bottom spacer to keep full height balance aligned with right column on large screens */}
          <div className="hidden lg:block" />
        </div>

        {/* Right Column: 4 Cards in 2x2 Grid with Bespoke Icons & Isometric 3D Artwork */}
        <div ref={cardsRef} className="lg:w-[62%] grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6 self-stretch">
          {t.services.items.map((service, index) => (
            <div
              key={index}
              className="group p-7 lg:p-8 rounded-2xl bg-[#121212] border border-white/[0.08] hover:border-brand-yellow/50 transition-all duration-300 relative overflow-hidden flex flex-col justify-between min-h-[290px] lg:min-h-[310px] hover:shadow-[0_12px_32px_rgba(0,0,0,0.5)]"
            >
              ...
              <div
                ...
                style={{
                  background: 'radial-gradient(circle at center, rgba(255,240,131,0.08) 0%, transparent 70%)',
                }}
              />
              ...
              <div className="flex justify-between items-start mb-5 relative z-10">
                ...
                <div className="w-10 h-10 rounded-full border border-brand-yellow/70 bg-brand-yellow/10 flex items-center justify-center shrink-0 group-hover:scale-105 group-hover:bg-brand-yellow/15 group-hover:border-brand-yellow transition-all duration-300 shadow-[0_0_15px_rgba(255,240,131,0.12)]">
                  {serviceBadgeIcons[index % serviceBadgeIcons.length]}
                </div>

                {/* Isometric 3D Illustration Thumbnail */}
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-2xl font-bold mb-3 tracking-tight text-white group-hover:text-brand-yellow transition-colors duration-300">
                  {service.title}
                </h3>
                ...
                <ul className="space-y-1.5">
                  {service.items.map((item, i) => (
                    <li key={i} className="text-sm text-brand-gray/90 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-brand-yellow/40 group-hover:bg-brand-yellow transition-colors duration-300 shrink-0" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
              ...
              {/* Interactive Diagonal Arrow (Appears on Hover, Mirrored for RTL) */}
              <div className="absolute bottom-6 right-6 rtl:right-auto rtl:left-6 text-brand-yellow opacity-40 group-hover:opacity-100 transform group-hover:translate-x-1 group-hover:-translate-y-1 rtl:group-hover:-translate-x-1 transition-all duration-300 pointer-events-none">
